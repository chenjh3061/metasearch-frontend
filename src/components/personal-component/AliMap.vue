<template>
  <div :style="`height: ${calcHeight('main')};`" class="wh-full pos-relative">
    ...
    <a-button v-show="isShowBack" class="map-back" @click="goBack"
      >返回上一级
    </a-button>
  </div>
</template>
...
import { useNotification } from "ant-design-vue/es/vc-notification";
...
const handleClick = (param: any) => {
  ...
  if (mapList.value[mapList.value.length - 1] === mapName) {
    //return notification.map({ content: "已经是最下层了", duration: 1000 });
  }
